...
**UI**: PrinterDetail's "Spoolman Spool" card, shown only while the integration is enabled. Unbound: a spool picker (from `GET /api/spoolman/spools?allow_archived=false`) plus a Bind button. Bound: the current material/color, plus Sync and Unbind buttons, and the Material/Color fields in the regular edit form become read-only with a hint pointing at the bound spool. A "Report usage to Spoolman" checkbox sets `spoolman_report_usage`, saved immediately like a normal field edit; it has no effect yet, the usage-tracking chunk is what will read it.
