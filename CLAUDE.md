...
- docs/proposals/ is a local-only folder of internal planning notes. It is deliberately never committed (it may be absent in a fresh clone), is not in the doc index, and is not current behavior. Do not commit it or link to it from committed docs. The one decision it holds that matters here: the client test framework (Vitest, happy-dom, React Testing Library, dev dependencies only) is built through Phase 3: pure-logic tests, the shared `client/src/lib/` helpers, and a DOM layer with a route smoke test. Playwright end-to-end tests are the one remaining deferred piece and need their own dependency approval. Do not add any other test dependency (for example `@testing-library/user-event` or jest-dom matchers) without asking Sean.
...
- Data, G-code and client build locations come from `server/paths.js` (`PFM_DATA_DIR`, `PFM_GCODE_DIR`, `PFM_CLIENT_DIST`, defaults unchanged). Never hard-code those paths. The server Jest suite runs every test file against a private scratch directory (`jest.config.js`), so `npm test` is safe on a machine with real data.
...
- Client tests (`client/tests/`, Vitest) cover the pure helpers in `client/src/lib/` (`format.js`, `printer-status.js`, `gcode-parse.js`), translation keys, `en.json`, the client/server input formats, the shared hooks and components, every route (mounted against a mocked API in `routes-smoke.test.jsx`), and the Jobs filters in the URL. DOM tests start with `// @vitest-environment happy-dom` and use the helpers in `client/tests/helpers/` (`t()` there throws on an unknown key; `mockApi` records unmocked requests, which fail the smoke test). Put new display logic that does not need React in `client/src/lib/` with a test, never copy a formatter or a status condition into a page. The suite runs in happy-dom, not a browser, so layout, styling and the WebGL viewer are still verified in a real browser (dev container, `DEMO_MODE=true`), and your summary says exactly which routes and interactions you checked.
...
| "Awaiting sign-off" logic: `isAwaitingSignoff` in client/src/lib/printer-status.js (used by Dashboard, Printers, and the Fleet cards) | There is one definition now; never re-inline it in a page. Fleet's bulk "Set Ready (N)" list uses the separate, deliberately narrower `isBatchReleasable`, which excludes `STOPPED`: a stopped plate defaults to 0 good parts and is credited only by an explicit per-printer confirmation, and `POST /api/printers/set-ready-batch` only clears the hold without resolving the stopped job. Widening the bulk list changes what one click can release, so it needs the completed_qty analysis first (escalation rule 1). `client/tests/printer-status.test.js` pins both. |
...
| Any code or test that reads or writes the database file, uploaded G-code, backups, or the client build | `server/paths.js` (`PFM_DATA_DIR`, `PFM_GCODE_DIR`, `PFM_CLIENT_DIST`); `server/tests/test-isolation.test.js` fails if a test could reach the real directories |
| A new inline endpoint in `server/index.js`, or a change to startup, static files, SPA fallback or middleware order | `server/tests/server-smoke.test.js` (route tests mount routers on a throwaway app and cannot see `index.js`) |
...
| A new route in `client/src/App.jsx`, or a new request a page makes when it loads | The `PAGES` list in `client/tests/routes-smoke.test.jsx`, and `baseRoutes()` in `client/tests/helpers/fixtures.js` (an unmocked request fails the smoke test on purpose) |
...
- **The heavyweight test.** Importing server/index.js or the real db.js in a test. Rule: tests build `new Database(':memory:')`, define the minimal schema inline, mount the route factory on a throwaway Express app, and drive it with supertest. Drivers mock the transport (`jest.mock('axios')`, mocked mqtt), never the driver module itself. There are two deliberate exceptions, both isolated by design: `server/tests/db-fresh-install.test.js` loads a copy of the real db.js into a scratch directory (migration ordering on a fresh database cannot be reproduced any other way), and `server/tests/server-smoke.test.js` runs the real `server/index.js` as a child process on its own `PFM_*` directories (nothing else exercises index.js: startup, static and SPA handling, the inline endpoints, an Express upgrade).
- **The real-database test.** Letting a test reach the real `server/data` or `server/gcode`. `server/events.js` opens the real database when merely required, so route tests that triggered events wrote fake ones ("Job Failed", "decommission", "Material: (none)") into the real printer history, attached to real printers with matching ids, and events are never deleted; upload tests left fixtures in the real G-code folder. Rule: tests never touch those directories. `jest.config.js` gives each test file a private scratch directory and `server/paths.js` throws under Jest if the real ones would be used. Never spell `server/data`, `server/gcode` or `client/dist` in code or tests: use `require('./paths')` / `require('../paths')`.
...
- **The fake browser pass.** Treating a green client test suite as proof the UI works. The DOM tests run in happy-dom, a lightweight DOM and not a browser: they catch a page that throws, an unexpected request, and logic errors, but not layout, styling, real browser behavior, or the WebGL viewer, and they only see the API responses the fixtures give them. Rule: for a visual or interaction change, still check it in a real browser and say what you checked.
...
- [ ] Layout, styling, interactions or the 3D viewer changed: browser-checked against the dev container with `DEMO_MODE=true`, and the summary names what was checked (the smoke test only proves pages mount)
