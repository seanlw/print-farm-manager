...
On the client, a Web Worker (`gcode-parser.worker.js`) parses `G0`/`G1`/`G2`/`G3` extrusion moves into a `Float32Array` of segment start/end positions off the main thread, so parsing doesn't freeze the UI regardless of file size; travel (non-extruding) moves are discarded entirely rather than rendered, so the preview only shows printed material. X/Y/Z positioning mode (`G90`/`G91`) and extruder mode (`M82`/`M83`) are tracked as two independent flags rather than one, since real slicer output routinely mixes them — PrusaSlicer's standard preamble is `G90` (absolute XYZ) followed by `M83` (relative E) — so E must accumulate onto a running total even while X/Y/Z stay absolute. `G92` (Set Position) is also handled — it redefines whichever axes it names to the literal value given, independent of relative/absolute mode, without producing a move; slicers commonly emit `G92 E0` periodically (even under `M83`) to reset the extruder's position register, and skipping it entirely leaves the tracked E value stale enough that the next real extrusion move can be misread as a non-extruding travel. Segments render as fat lines (`LineSegments2`/`LineSegmentsGeometry`/`LineMaterial` from `three/addons/lines`) with a constant **screen-space** line width (`LINE_WIDTH_PX = 5`, in CSS pixels, `worldUnits: false`) rather than a world-space (mm) size. A fixed pixel width stays visually constant regardless of zoom or model scale, so a real slicer's actual travel gaps (perimeters broken into passes, infill lines separated by travel moves, etc.) mostly disappear into the line's own thickness at typical part-viewing zoom rather than reading as visible holes. Zooming in far enough, or a travel move large enough (a multi-millimeter perimeter-to-perimeter jump, a bed-spanning prime line), can still show a real gap once it exceeds the line's width at that scale; this is an accurate reflection of the underlying toolpath, not a rendering defect. Vertex-colored by height (blue → yellow) via `LineSegmentsGeometry.setColors`. `OrbitControls` handles mouse-driven rotate/zoom/pan; none of this reaches for a React-three renderer layer, matching this app's existing hand-rolled style. A bottom-right camera control pad — an SVG D-pad matching the layout found in most CAD/model-viewer UIs (e.g. printables.com's viewer) — adds four rotate wedges (nudging the camera around its target by a fixed 15° step, via spherical-coordinate orbiting clamped away from the poles to avoid `OrbitControls`' gimbal-lock instability at a perfectly vertical look direction), a center zoom in/out split circle, and two icon buttons below: Reset (returns to the default framed view and zoom) and an isometric-cube icon (reorients to the standard angle but keeps the current zoom level, unlike Reset). Files over 100 MB are blocked by default (with a "Try anyway" override); 20–100 MB shows a performance warning — a new `file_size` column (populated from multer's already-available `req.file.size`, previously discarded) makes this possible without an extra request.

The parser also tracks each section's `;TYPE:<feature>` comment — a convention PrusaSlicer (and Bambu/Orca, which share its lineage) uses to tag every part of the G-code — and excludes `Custom` (custom start/end G-code, including the nozzle-priming "intro line" drawn away from the part) and `Skirt/Brim` (the loop printed around the part's footprint before the real print starts) from the rendered geometry. Neither is part of the physical part being previewed, and dropping them has a second benefit beyond a cleaner render: the camera's framing is computed from the rendered geometry's bounds, so a prime line drawn well outside the part's footprint was previously pulling those bounds wide and leaving the actual model small and off-center in the default view.

`file_size` is now backfilled lazily rather than left permanently `NULL` for any `gcodes` row that predates the column (the `ALTER TABLE` that added it doesn't populate existing rows, and nothing else ever wrote to it after the fact). `GET /api/gcodes` stats the on-disk file for any row it returns with a `NULL` `file_size` and persists the result, so each row self-heals the first time it's actually listed — chosen over a startup migration loop, which would scale with however large the pre-existing backlog is and block the entire server (including printer polling) until it finished for however many old rows exist. A row that already has a `file_size` is never re-stat'd or overwritten, and a row whose file is missing from disk is left `NULL` rather than erroring. Without this, an operator opening the 3D Viewer on a G-code uploaded before this feature shipped would get no size warning at all — the client's `tooLarge`/`showWarning` checks both require `file_size != null` — regardless of how large the file actually is.

`GET /api/gcodes/:id/preview` treats any decode failure — not just the explicitly-typed `GcodeDecodeError` cases — as a `422` with a `DECODE_FAILED` code, logging the underlying error server-side. A structurally valid bgcode file (passes the magic header and every block-bounds check) with a corrupt payload for an otherwise-recognized compression type causes `pako`/`heatshrink-ts` to throw a plain `Error` rather than a `GcodeDecodeError`; on Express 4, an uncaught error thrown from an async handler becomes an unhandled promise rejection, and this app's top-level `unhandledRejection` handler treats any of those as fatal and exits the process — so a single malformed uploaded file previously took down the whole server for every user, not just the one preview request.

On unmount (or when re-rendering for a different G-code), `GcodeViewerModal` now explicitly disposes every geometry and material in the scene graph in addition to the renderer and controls. `WebGLRenderer.dispose()` only frees the renderer's own internal state (program cache, render lists, bindings) — it does not free geometries or materials attached to scene objects, so the `LineSegmentsGeometry`/`LineMaterial` backing the fat-line render (position and color buffers sized to the segment count — tens of thousands of segments for a typical part) were leaking GPU memory on every viewer close, with no bound other than closing the browser tab.

`GET /api/gcodes?part_id=` now orders results oldest-first (`created_at ASC`, `id` as a tiebreak for same-millisecond inserts) instead of leaving row order unspecified. The 3D Viewer previews `gcodes[0]` as a part's representative file, and the client applies no ordering of its own, so this endpoint's result order is what actually determines which file gets previewed.

Decompression is now capped at 200MB of output (`MAX_DECOMPRESSED_BYTES` in `server/gcode-decode.js`), enforced independent of anything a file claims about its own size. DEFLATE and Heatshrink's compression ratio scales with how repetitive the input is, not with the compressed size — a few hundred KB on disk can expand to hundreds of MB to GB in memory in well under a second, and neither `pako.inflateRaw` nor `heatshrink-ts`'s one-shot `.process()` call offered any way to find out how large the output was going to be before it was already fully decompressed. Deflate now runs through pako's streaming `Inflate` API and Heatshrink is fed its compressed input in small chunks, both draining and checking cumulative output after each step so a bomb is caught within the first chunk or two rather than after paying for the whole thing; `.3mf` reads its zip entry the same way via JSZip's streaming `internalStream`. A file with many blocks that each individually stay under the cap but sum past it is also caught, via a running total checked in `decodeBgcode`'s main loop. Multer's upload handler also now has a `250MB` `limits.fileSize` (`server/routes/gcodes.js`) — it had none before — bounding per-upload disk usage, independent of the decompression cap.

`GcodeViewerModal`'s `OrbitControls` now sets `minDistance`/`maxDistance` (its library defaults are `0`/`Infinity` — no clamp at all). Without this, both the NavPad's zoom buttons and native mouse-wheel/pinch zoom could push the camera past the camera's own near or far clipping plane: zooming out far enough clipped the model out of view entirely (a blank viewport indistinguishable from a stuck load), and zooming in far enough put the camera inside the geometry, rendering a chaotic, unusable close-up. Reset already recovered from either case, but nothing on screen indicated that's what had happened.

`decodeMeatpack` collects its output into bounded chunks (flushed and joined together every 65536 characters) rather than one array growing by a single character per `.push()` call. A plain JS array throws `RangeError: Invalid array length` once pushed past roughly 113 million elements — a real, directly-confirmed V8 engine ceiling — which sits below the 200MB decompression cap a Deflate/Heatshrink-compressed MeatPack stream can legitimately expand to once unpacked; a large but entirely legitimate MeatPack-encoded file could have crashed the decode (caught as a 422 by the decode-failure handling above rather than taking the server down, but reported misleadingly as "may be corrupt" for a file that wasn't). The uncompressed (`COMPRESSION_NONE`) block path is now also capped for consistency, even though it carries no amplification risk of its own — its input size is already bounded by the upload limit.

`GcodeViewerModal` resets its `proceedAnyway` ("Try anyway" past the large-file block) state whenever `gcode.id` changes. The component isn't remounted on a G-code change — there's no `key` tied to `gcode.id` in `Projects.jsx`, and a part's underlying G-code list can change (a file deleted and re-uploaded via the Details panel) while the viewer stays open — so without this, a "Try anyway" decision made for one large file would silently carry over to a completely different file the operator never actually confirmed. Reset happens synchronously during render (React's documented pattern for adjusting state on a prop change) rather than in an effect, so `tooLarge` is never computed from the stale value even for one render.

Verified: full suite passes, including regression coverage for every bgcode compression/encoding combination against both synthetic and real fixtures, `.3mf` extraction, the new route's 404/422/200 paths, a corrupt-payload-with-known-compression-type case that previously bypassed the typed-error check, the `GET /api/gcodes?part_id=` ordering guarantee (including the same-timestamp tiebreak), and a real decompression-bomb reproduction for both `.bgcode` (Deflate) and `.3mf` (zip) — a few hundred KB of input that would otherwise expand past 200MB, asserted to be rejected in well under a second rather than fully inflated first. Also verified live against the running dev server: uploaded a real PrusaSlicer-generated `.bgcode` file, a plain `.gcode` file, and a hand-built `.3mf` wrapping the same real G-code, and confirmed all three still render as continuous, correctly-colored models (rotate/zoom, no travel-move clutter) through the actual browser UI after the decompression-cap changes, not just the decoder in isolation; also repeatedly closed and reopened the viewer to exercise the dispose/re-create cycle, with no console errors or visual regressions. The `G92` fix was checked against a synthetic absolute-extrusion-mode file with a mid-print `G92 E0` reset, confirming the extrusion move immediately following the reset is now correctly kept rather than dropped, and against the real fixture (unaffected, since it uses relative extrusion throughout). The camera-clamp fix was verified by dispatching real clicks on the NavPad's zoom controls (not estimated): 20 zoom-out clicks and 60 zoom-in clicks each leave the model visible and stable, where before the fix 12 zoom-out clicks blanked it and 40 zoom-in clicks broke into the geometry's interior. The `decodeMeatpack` array-length fix was reproduced directly against the pre-fix code (confirming the exact `RangeError: Invalid array length` and the ~113 million element threshold it throws at) and against the fix (a 150MB case that used to crash now decodes correctly, byte-for-byte, in a few seconds); the committed regression test uses a much smaller input that still spans several internal chunk-flush boundaries, since reproducing the full-scale case in the suite runs into the same pako-under-Jest slowdown noted below and would add tens of seconds to the test run for no additional coverage. The `proceedAnyway` reset was verified live: reopened the viewer after the fix and confirmed no console errors (in particular, no "Maximum update depth exceeded" from the render-time state adjustment) and normal rendering. The `;TYPE:` exclusion was checked against the real Bag fixture directly (84,550 segments before → 84,375 after, matching the expected intro-line-plus-skirt-loop reduction) and confirmed live in the browser: the prime line visible in every prior screenshot of the bgcode/3mf test parts is gone, and — as a direct consequence of bounds no longer including it — those parts now frame correctly in the default view instead of rendering small and off-center. The lazy `file_size` backfill was verified with three cases: a `NULL` row with a real file on disk gets the correct size on the very next `GET`, persisted (not just reflected in that one response); a `NULL` row whose file is missing from disk stays `NULL` without erroring; and a row that already has a `file_size` — deliberately set to a value that doesn't match its real on-disk size — is confirmed completely untouched (no re-stat, no overwrite) after a `GET`. No client-side test runner exists in this repo, so the viewer itself (rendering, controls, size-guard UI) was verified manually rather than via automated tests, matching how other client-side changes in this project are verified.

(Aside, for anyone touching these decode paths later: both new decompression-bomb tests, and the `decodeMeatpack` regression test above, were originally written at full scale — hundreds of MB of real work — and ran in seconds in plain Node but took 24-30 seconds each under Jest's instrumentation specifically for `pako`'s pure-JS compression and for tight per-character loops. Building the Deflate test fixture with Node's native `zlib` instead of `pako`, and shrinking the `decodeMeatpack` test to a size that still exercises the mechanism without needing to reach the original crash threshold, brought the whole suite back to its normal ~4s.)
