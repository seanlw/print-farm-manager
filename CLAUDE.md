...
- docs/proposals/ is a local-only folder of internal planning notes. It is deliberately never committed (it may be absent in a fresh clone), is not in the doc index, and is not current behavior. Do not commit it or link to it from committed docs. The one decision it holds that matters here: the client test framework (Vitest, dev dependencies only) is being built in phases. Phases 1 and 2 are done (Vitest, `client/src/lib/{format,printer-status,gcode-parse}.js`, `client/tests/`). Still planned and approved: a thin DOM layer (happy-dom and React Testing Library, added when that phase starts). Playwright end-to-end tests are deferred. Do not add a dependency beyond Vitest, happy-dom, `@testing-library/react` and `@testing-library/dom` without asking Sean.
...
- Client tests cover pure logic only: the helpers in `client/src/lib/` (`format.js`, `printer-status.js`, `gcode-parse.js`), translation keys, `en.json`, and the client/server input formats. Put new display logic that does not need React in `client/src/lib/` with a test in `client/tests/`, never copy a formatter or a status condition into a page. Pages, hooks and the WebGL viewer have no automated tests yet, so verify those in a browser (dev container, `DEMO_MODE=true`) and say in your summary exactly which routes and interactions you checked.
