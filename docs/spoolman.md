...
While enabled, the Filament Library section in Settings renders read-only and hides its manual Add Type/Add Color forms rather than deleting the underlying local tables, so disabling the integration instantly restores manual editing. Rather than listing every filament (a farm's Spoolman library can get large), it shows a one-line count summary (for example "12 filaments in Spoolman.") alongside a "Manage in Spoolman" link that opens `spoolman_base_url` directly in a new tab (`target="_blank"`) for adding vendors, filaments, or spools, since none of that management happens in this app. This assumes the same URL is reachable from both the server (which already proxies through it) and the operator's browser, true for any normal self-hosted Spoolman instance on the farm's LAN.
