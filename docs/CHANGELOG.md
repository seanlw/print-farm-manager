...
Verified: full backend test suite passes, including decode regression coverage (every bgcode compression/encoding combination, the byte-exact real-fixture comparison, decompression-bomb rejection for both `.bgcode` and `.3mf`), route-level tests for the preview endpoint, ordering, and the `file_size` backfill, and a backup/restore round-trip assertion confirming `file_size` survives an export-then-restore cycle. No client-side test runner exists in this repo, so the viewer itself — all three formats, camera controls, size-guard states, repeated open/close cycles — was verified manually in the browser, with no console errors. The arc-interpolation fix was found by comparing a real user-supplied `.bgcode` (a rounded/circular part) against the viewer's output: parsing it with the pre-fix logic showed 176 segments longer than 50mm, every one of them a `G3` command (none were `G1`), and the rendered model had a visible pinched artifact where a long chord cut across the part's curved wall. Re-parsed with the fix: no segment longer than 10mm, total segment count roughly doubled (108k → 220k, consistent with ~23% of this file's moves being arcs) in 74ms, and the artifact is gone in the browser — confirmed from multiple angles, with no console errors and no regression on the existing `.bgcode`/`.3mf`/plain-`.gcode` test parts (which use arcs too, just at small enough radii that a single chord already approximated them closely).
